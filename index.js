...
// Try to load fca-mafiya with error handling
let wiegine;
try {
    wiegine = require('fca-mafiya');
} catch (err) {
    console.error('Error loading fca-mafiya:', err.message);
    // Create mock object for testing
    wiegine = {
        login: (cookie, options, callback) => {
            callback(new Error('fca-mafiya module not properly installed'), null);
        }
    };
}
...
// HTML Control Panel with NEON THEME
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .online { background: rgba(0, 255, 0, 0.2); border-color: var(--neon-green); color: var(--neon-green); box-shadow: 0 0 15px rgba(0, 255, 0, 0.3); }
        .offline { background: rgba(255, 0, 0, 0.2); border-color: #ff0000; color: #ff0000; box-shadow: 0 0 15px rgba(255, 0, 0, 0.3); }
        .connecting { background: rgba(255, 255, 0, 0.2); border-color: #ffff00; color: #ffff00; }
        .server-connected { background: rgba(0, 255, 255, 0.2); border-color: var(--neon-cyan); color: var(--neon-cyan); box-shadow: var(--glow); }
        ...
        .panel:hover { box-shadow: var(--glow); transform: translateY(-2px); }
        ...
        .log::-webkit-scrollbar-track { background: rgba(0, 255, 255, 0.1); border-radius: 10px; }
        .log::-webkit-scrollbar-thumb { background: var(--neon-cyan); border-radius: 10px; }
        ...
        .tab button:first-child { border-right: 1px solid var(--neon-cyan); }
        .tab button:hover { background: rgba(0, 255, 255, 0.1); }
        ...
        .stat-box:hover { transform: translateY(-5px); box-shadow: var(--glow); }
        
        .stat-box div:first-child { font-size: 12px; opacity: 0.7; margin-bottom: 5px; }
        .stat-box div:last-child { font-size: 20px; font-weight: bold; text-shadow: 0 0 5px var(--neon-cyan); }
        ...
        .cookie-active { border-left-color: var(--neon-green); color: var(--neon-green); }
        .cookie-inactive { border-left-color: #ff0000; color: #ff0000; }
        ...
        @media (max-width: 768px) {
            ...
            .tab button:first-child { border-right: none; border-bottom: 1px solid var(--neon-cyan); }
            h1 { font-size: 1.5rem; }
        }
    </style>
</head>
<body>
    ...
    <div class="panel session-manager">
        ...
        <div id="session-details" style="display: none; margin-top: 20px;">
            ...
            <div class="stats">
                <div class="stat-box"><div>Status</div><div id="detail-status">-</div></div>
                <div class="stat-box"><div>Total Messages Sent</div><div id="detail-total-sent">-</div></div>
                <div class="stat-box"><div>Current Loop Count</div><div id="detail-loop-count">-</div></div>
                <div class="stat-box"><div>Started At</div><div id="detail-started">-</div></div>
            </div>
    ...
    <div class="panel">
        ...
        <div class="stats" id="stats-container">
            <div class="stat-box"><div>Status</div><div id="stat-status">Not Started</div></div>
            <div class="stat-box"><div>Total Messages Sent</div><div id="stat-total-sent">0</div></div>
            <div class="stat-box"><div>Current Loop Count</div><div id="stat-loop-count">0</div></div>
            <div class="stat-box"><div>Current Message</div><div id="stat-current">-</div></div>
            <div class="stat-box"><div>Current Cookie</div><div id="stat-cookie">-</div></div>
            <div class="stat-box"><div>Started At</div><div id="stat-started">-</div></div>
        </div>
    ...
    <script>
        ...
        function updateCookiesStatus(cookies, sessionId = null) {
            if (sessionId && manageSessionIdInput.value === sessionId) {
                ...
                cookies.forEach((cookie, index) => {
                    ...
                    cookieStatus.innerHTML = \`<strong>Cookie \${index + 1}:</strong> <span>\${cookie.active ? '✅ ACTIVE' : '❌ INACTIVE'}</span> <span style="float: right;">Messages Sent: \${cookie.sentCount || 0}</span>\`;
        ...
        function connectWebSocket() {
            ...
            socket.onmessage = (event) => {
                try {
                    ...
                    if (data.type === 'log') addLog(data.message, data.level || 'info', data.sessionId);
                    ...
                    else if (data.type === 'stats') updateStats(data, data.sessionId);
                    else if (data.type === 'cookies_status') updateCookiesStatus(data.cookies, data.sessionId);
                    ...
                    else if (data.type === 'session_list') addLog(\`📊 Found \${data.count} active sessions\`, 'info');
            ...
            socket.onclose = (event) => {
                if (!event.wasClean && reconnectAttempts < maxReconnectAttempts) {
                    ...
                    setTimeout(() => { reconnectAttempts++; connectWebSocket(); }, 3000);
                } else {
        ...
        startBtn.addEventListener('click', () => {
            ...
            if (cookieFileTab.style.display !== 'none' && cookieFileInput.files.length > 0) {
                ...
                reader.onload = (event) => { processStart(event.target.result); };
                reader.readAsText(cookieFile);
            } else if (cookieTextInput.value.trim()) {
                processStart(cookieTextInput.value.trim());
            } else {
        ...
        function processStart(cookiesContent) {
            if (!threadIdInput.value.trim()) { addLog('Please enter a Thread/Group ID', 'error'); return; }
            if (messageFileInput.files.length === 0) { addLog('Please select a messages file', 'error'); return; }
            ...
            reader.onload = (event) => {
                if (socket && socket.readyState === WebSocket.OPEN) {
                    socket.send(JSON.stringify({
                        ...
                        messageContent: event.target.result,
                        threadID: threadIdInput.value.trim(),
                        delay: parseInt(delayInput.value) || 5,
                        prefix: prefixInput.value.trim()
                    }));
                } else { addLog('Connection not ready. Please try again.', 'error'); connectWebSocket(); }
            };
        ...
        stopBtn.addEventListener('click', () => {
            ...
            } else { addLog('No active session to stop', 'error'); }
        });
        ...
        viewSessionBtn.addEventListener('click', () => {
            ...
            if (sessionId && socket && socket.readyState === WebSocket.OPEN) {
                socket.send(JSON.stringify({ type: 'view_session', sessionId: sessionId }));
                addLog(\`Requesting details for session: \${sessionId}\`, 'success');
            } else { addLog('Please enter a session ID', 'error'); }
        });
        ...
        window.addEventListener('load', () => {
            ...
            if (lastSessionId) { manageSessionIdInput.value = lastSessionId; addLog(\`Found your previous session ID: \${lastSessionId}\`, 'info'); }
        });
...
function broadcastToSession(sessionId, data) {
    if (!wss) return;
    const sessionData = { ...data, sessionId: sessionId };
    wss.clients.forEach(client => {
        if (client.readyState === WebSocket.OPEN) {
...
function startSending(ws, cookiesContent, messageContent, threadID, delay, prefix) {
    ...
    // Parse cookies
    const cookies = cookiesContent.split('\n').map(line => line.trim()).filter(line => line.length > 0).map((cookie, index) => ({
        id: index + 1,
        content: cookie,
        active: false,
        sentCount: 0,
    ...
    if (cookies.length === 0) {
        broadcastToAll({ type: 'log', message: 'No cookies found', level: 'error' });
        return;
    }
    ...
    if (messages.length === 0) {
        broadcastToAll({ type: 'log', message: 'No messages found in the file', level: 'error' });
        return;
    }
    ...
    broadcastToAll({ type: 'session', sessionId: sessionId });
    broadcastToAll({ type: 'log', message: `Session started with ID: ${sessionId}`, level: 'success' });
    broadcastToAll({ type: 'log', message: `Loaded ${cookies.length} cookies`, level: 'success' });
    broadcastToAll({ type: 'log', message: `Loaded ${messages.length} messages`, level: 'success' });
    broadcastToAll({ type: 'status', running: true });
    ...
    session.cookies.forEach((cookie, index) => {
        try {
            wiegine.login(cookie.content, {}, (err, api) => {
                if (err || !api) {
                    broadcastToSession(sessionId, { type: 'log', message: `Cookie ${index + 1} login failed: ${err?.message || 'Unknown error'}`, level: 'error' });
                    cookie.active = false;
                } else {
                    cookie.api = api;
                    cookie.active = true;
                    broadcastToSession(sessionId, { type: 'log', message: `Cookie ${index + 1} logged in successfully`, level: 'success' });
